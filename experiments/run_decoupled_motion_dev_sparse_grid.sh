...
RUN_LABEL="${RUN_LABEL:-decoupled_motion_dev_sparse_grid}"
...
MOTION_PROFILES_CSV="${MOTION_PROFILES:-base,strong}"

METHOD_NAME="动态共享-解耦运动潜码"
...
motion_profiles=$MOTION_PROFILES_CSV
manifest=$MANIFEST
batch_dir=$BATCH_DIR
EOF

echo "[DecoupledSparseGrid] batch_dir=$BATCH_DIR"

IFS=',' read -r -a CORR_SCHEDULES <<< "$CORR_SCHEDULES_CSV"
IFS=',' read -r -a BOOTSTRAP_PROFILES <<< "$BOOTSTRAP_PROFILES_CSV"
IFS=',' read -r -a MOTION_PROFILES <<< "$MOTION_PROFILES_CSV"
...
append_bootstrap_profile_args() {
  ...
  case "$profile" in
    ...
    strong)
      out_args_ref+=(
        --bootstrap-offset-weight 0.35
        --bootstrap-decay-fraction 0.45
        --unsupported-anchor-weight 0.10
        --unsupported-laplacian-weight 0.20
      )
      ;;
    *)
      echo "[DecoupledSparseGrid] Unknown bootstrap profile: $profile" >&2
      exit 1
      ;;
  esac
}

append_motion_profile_args() {
  local profile="$1"
  local -n out_args_ref=$2
  case "$profile" in
    base)
    ...
    strong)
      out_args_ref+=(
        ...
        --motion-mean-weight 0.10
        --motion-lipschitz-weight 0.05
        --smoothing-iterations 12
      )
      ;;
    *)
      echo "[DecoupledSparseGrid] Unknown motion profile: $profile" >&2
      exit 1
      ;;
  esac
}

echo 'config_name,correspondence_start_fraction,correspondence_ramp_fraction,bootstrap_profile,motion_profile' > "$GRID_CONFIG_PATH"
...
for corr_schedule in "${CORR_SCHEDULES[@]}"; do
  ...
  for bootstrap_profile in "${BOOTSTRAP_PROFILES[@]}"; do
    for motion_profile in "${MOTION_PROFILES[@]}"; do
      config_name="cs$(sanitize_number "$corr_start")_cr$(sanitize_number "$corr_ramp")_${bootstrap_profile}_${motion_profile}"
      echo "$config_name,$corr_start,$corr_ramp,$bootstrap_profile,$motion_profile" >> "$GRID_CONFIG_PATH"
      config_count=$((config_count + 1))
    done
  done
done

echo "[DecoupledSparseGrid] config_count=$config_count"

run_count=0
config_index=0
while IFS=, read -r config_name corr_start corr_ramp bootstrap_profile motion_profile; do
  ...
  while IFS=, read -r instance_name shape_family split condition reference_ply monitor_stream scanner_sequence phase_model_dir phase_summary condition_root condition_metadata; do
    ...
    run_name="${instance_name}_decoupled_motion_dev_sparse_cfg_${config_name}"
    common_args=(
      --mode "$MODE"
      --method "$METHOD_NAME"
    ...
    append_motion_profile_args "$motion_profile" common_args

    echo "[DecoupledSparseGrid] instance=$instance_name config=$config_name corr=${corr_start}/${corr_ramp} bootstrap=$bootstrap_profile motion=$motion_profile"
    if [[ "$DRY_RUN" == "1" ]]; then
      printf '[DecoupledSparseGrid][DRY_RUN] %q ' "$PYTHON_BIN" "$ROOT_DIR/scripts/run_single_dynamic_shared.py" "${common_args[@]}"
      printf '\n'
      continue
    fi
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[DecoupledSparseGrid] dry run complete"
  exit 0
fi

echo "[DecoupledSparseGrid] completed_runs=$run_count"
...
instance_df = instance_df[instance_df["method"] == "动态共享-解耦运动潜码"].copy()
...
formatted_df = summary_df[["rank", "config_name", "correspondence_start_fraction", "correspondence_ramp_fraction", "bootstrap_profile", "motion_profile"]].copy()
...
print(f"[DecoupledSparseGrid] Config summary: {agg_dir / 'config_level_summary.csv'}")
PY

echo "[DecoupledSparseGrid] done"