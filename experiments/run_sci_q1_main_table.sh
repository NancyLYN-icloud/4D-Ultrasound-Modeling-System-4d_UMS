...
tail -n +2 "$MANIFEST" | while IFS=, read -r instance_name shape_family split condition reference_ply monitor_stream scanner_sequence phase_model_dir phase_summary condition_root condition_metadata; do
  ...
  if [[ ! -d "$gt_mesh_dir" ]]; then
    echo "[SCI-Q1] Skip $instance_name: GT mesh dir missing -> $gt_mesh_dir" >&2
    continue
  fi
  if [[ ! -f "$monitor_stream" ]]; then
    echo "[SCI-Q1] Skip $instance_name/$condition: monitor stream missing -> $monitor_stream" >&2
    continue
  fi
  if [[ ! -f "$scanner_sequence" ]]; then
    echo "[SCI-Q1] Skip $instance_name/$condition: scanner sequence missing -> $scanner_sequence" >&2
    continue
  fi
...
echo "[RobustnessBenchmark] completed batch_dir=$BATCH_DIR"