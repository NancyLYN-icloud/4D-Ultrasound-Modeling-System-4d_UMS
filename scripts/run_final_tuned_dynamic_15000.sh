...
run_method() {
    ...
    echo "[Final15000] Running: $method ($run_name)"
...
run_method "动态共享-参考对应正则" "refcorr-smallnet-delayed-15000-final" \
...
run_method "动态共享-连续形变场" "continuous-history-lite-15000-final" \
    --dynamic-mesh-resolution 72 \
    --max-points-per-phase 3000 \
...
run_method "动态共享-解耦运动潜码" "decoupled-motion-balanced-15000-final" \
    --dynamic-mesh-resolution 72 \
    --max-points-per-phase 3000 \
    --confidence-floor 1.0 \
    --temporal-weight 0.02 \
